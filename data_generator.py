...
def generate_dataset(name, num_sites, seed=42):
    """
    Generate dataset for microgrid optimization using validated Ethiopia data.
    
    Args:
        name (str): Dataset name
        num_sites (int): Number of sites to return (max 50)
        seed (int): Random seed (unused, kept for API compatibility)
    
    Returns:
        pd.DataFrame: DataFrame with site data
    """
    df = pd.DataFrame(VALIDATED_SITES)
    
    if num_sites < len(df):
        df = df.head(num_sites)
...
def generate_ethiopia_dataset(num_sites=50, seed=42):
    """
    Generate Ethiopia-specific dataset with real coordinates and validated data.
    ...
    """
    return generate_dataset("Ethiopia_Offgrid_Real", num_sites, seed)


if __name__ == "__main__":
    # Generate and display sample dataset
    df = generate_ethiopia_dataset(50)
    print("Generated Ethiopia Dataset (Validated):")
    print(df.head(10))
    ...
    print(f"\nSolar Potential Range: {df['Solar_Potential_kWh_m2_day'].min():.2f} - {df['Solar_Potential_kWh_m2_day'].max():.2f} kWh/m²/day")
    print(f"Cost Range: ${df['Installation_Cost_USD'].min():,} - ${df['Installation_Cost_USD'].max():,}") 